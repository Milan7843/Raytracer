...
Sphere spheres[1] = Sphere[1](
    //     Pos                  Size    Color                   Reflectiveness      Fuzziness
    Sphere(vec3(0., -3., 0.),    1.2,    vec3(0.3, 0.6, 0.6),    0.0,                0.04, false)
);
...
Box boxes[1] = Box[1](
    ...
    Box(vec3(-1., 3., 5.),  vec3(1., 0.5, 1.), vec3(0.3, 0.6, 0.6), 0.1, false)
    );
...
RayResult fireRay(vec3 pos, vec3 direction)
{
    ...
    for (int i = 0; i < 50; i++)
    {
        ...
        for (int j = 0; j < triangles.length(); j++)
        {
            if (!trisReflected[j])
            {
                float dst = triangleDistance(rayPos, triangles[j]);
                //float dst = 5.;
                if (dst < closestDst)
                {
                    closestDst = dst;
                ...
                if (dst < dstThreshold)
                {
                    if (triangles[j].reflectiveness > 0.)
                    {
                        ...
                        timesReflected += triangles[j].reflectiveness;
                        endColor += triangles[j].color * triangles[j].reflectiveness;
                    }
                    else
                    {
                        hit = true;
